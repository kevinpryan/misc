...
def parse_args(args=None):
    Description = "Generate nf-core/hlatyping samplesheet from a directory of FastQ files."
    ...
    parser.add_argument(
        ...
        dest="SEQTYPE",
        default="rna",
        help="Type of sequencing, either dna or rna",
    )
    parser.add_argument(
        "-r1",
        "--read1_extension",
...
def fastq_dir_to_samplesheet(
    ...
    if len(read_dict) > 0:
        ...
        with open(samplesheet_file, "w") as fout:
            header = ["sample", "fastq_1", "fastq_2", "seq_type"]
            ...
            for sample, reads in sorted(read_dict.items()):
                for idx, read_1 in enumerate(reads["R1"]):
                    ...
                    if idx < len(reads["R2"]):
                        read_2 = reads["R2"][idx]
                    sample_info = ",".join([sample, read_1, read_2, seq_type])
